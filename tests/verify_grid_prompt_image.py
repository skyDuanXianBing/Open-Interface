...
def _build_screenshot_fixture(size: tuple[int, int]) -> Image.Image:
    ...
    draw.rectangle((20, 20, max(21, width - 20), max(21, height - 20)), outline=(120, 140, 180), width=2)
    draw.rectangle((40, 80, max(41, min(width - 40, 640)), max(81, min(height - 80, 260))), fill=(255, 255, 255))
    draw.rectangle((60, 110, max(61, min(width - 60, 580)), max(111, min(height - 110, 170))), outline=(80, 80, 80), width=2)
    draw.rectangle((max(41, width - 260), max(41, height - 120), max(42, width - 80), max(42, height - 60)), fill=(24, 118, 242))
    return image
...
def _install_pyautogui_stub() -> None:
    ...
    module.screenshot = lambda *args, **kwargs: _build_screenshot_fixture(CURRENT_SCREENSHOT_SIZE)
    for name in (
        'moveTo',
        'click',
        'doubleClick',
...
def _read_rgb_pixel(image: Image.Image, x_position: int, y_position: int) -> tuple[int, int, int]:
    ...
    *,
    ...
    with ExitStack() as stack:
        ...
        if archive_dir is not None:
            stack.enter_context(
                patch.object(Screen, '_get_prompt_image_archive_directory', return_value=archive_dir)
            )
...
def test_normal_path_grid_only_payload_and_archive() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert_true(len(archived_files) == 1, 'Expected exactly one archived prompt image.')
        archived_path = archived_files[0]

        violations: list[str] = []
        if TIMESTAMP_PATTERN.match(archived_path.name) is None:
            violations.append('Archive filename should use MM月DD日_HH时MM分SS秒 format.')
        if frame_context.get('model_prompt_image_path') != str(archived_path):
            violations.append('frame_context.model_prompt_image_path should point to archived prompt image.')

        encoded_bytes = base64.b64decode(payload['annotated_image_base64'])
        if archived_path.read_bytes() != encoded_bytes:
            violations.append('Archived image should be the final prompt image before base64 encoding.')

        if 'grid_reference' not in frame_context:
            violations.append('frame_context should include grid_reference for pure grid coordinate contract.')
        disallowed_fields = [
            'anchors',
        ...
        if leaked_fields:
            violations.append(f'Pure grid payload must not expose semantic fields: {leaked_fields}')
...
def test_boundary_small_screen_must_still_expose_percent_tick_contract() -> None:
    ...
    if not isinstance(grid_reference, dict):
        violations.append('Boundary case should still provide grid_reference dictionary.')
    else:
        ...
        x_3_percent = origin_x + int(round(int(captured_screen.get('width') or 0) * 0.03))
        x_99_percent = origin_x + int(round(int(captured_screen.get('width') or 0) * 0.99))
        y_3_percent = origin_y + int(round(int(captured_screen.get('height') or 0) * 0.03))
        y_99_percent = origin_y + int(round(int(captured_screen.get('height') or 0) * 0.99))

        if not _is_dark_red(_read_rgb_pixel(final_prompt_image, x_3_percent, origin_y + 10)):
            violations.append('3% vertical grid line should render as dark red.')
        if not _is_dark_red(_read_rgb_pixel(final_prompt_image, origin_x + 10, y_3_percent)):
            violations.append('3% horizontal grid line should render as dark red.')
        if not _is_dark_red(_read_rgb_pixel(final_prompt_image, x_99_percent, origin_y + 10)):
            violations.append('99% vertical grid line should render as dark red.')
        if not _is_dark_red(_read_rgb_pixel(final_prompt_image, origin_x + 10, y_99_percent)):
            violations.append('99% horizontal grid line should render as dark red.')
        if not _is_dark_red(_read_rgb_pixel(final_prompt_image, content_right, origin_y + 10)):
            violations.append('100% vertical endpoint tick should still render as dark red.')
        if not _is_dark_red(_read_rgb_pixel(final_prompt_image, origin_x + 10, content_bottom)):
            violations.append('100% horizontal endpoint tick should still render as dark red.')
    ...
    if (
        final_prompt_image.width == int(captured_screen.get('width', 0))
        and final_prompt_image.height == int(captured_screen.get('height', 0))
    ):
        violations.append('Final prompt image dimensions should reflect prompt rendering result, not raw capture dimensions.')
    ...
    payload = _build_payload(
        ...
        extra_patches=[
            patch(
                ...
                side_effect=AssertionError('OCR backend must not be called in pure grid pipeline.'),
            )
        ],
    )

    frame_context = payload['frame_context']
    assert_true('ocr_text_blocks' not in frame_context, 'Negative path: ocr_text_blocks must be removed from frame_context.')
    assert_true('semantic_regions' not in frame_context, 'Negative path: semantic_regions must be removed from frame_context.')
...
def test_sqlite_save_model_prompt_images_persistence() -> None:
    with tempfile.TemporaryDirectory() as temp_home:
        ...
        try:
            settings = Settings()
            first_saved = settings.save_settings({'save_model_prompt_images': True})
            assert_true(
                first_saved.get('save_model_prompt_images') is True,
                'save_settings should persist save_model_prompt_images=True.',
            )

            second_saved = settings.save_settings({'save_model_prompt_images': False})
            assert_true(
                second_saved.get('save_model_prompt_images') is False,
                'save_settings should persist save_model_prompt_images=False.',
            )

            reloaded = Settings().get_dict()
            assert_true(
                reloaded.get('save_model_prompt_images') is False,
                'save_model_prompt_images should be reloaded from SQLite.',
            )
        finally:
...
TEST_CASES = [
    ...
    ('Category 2 - Boundary Conditions', test_boundary_small_screen_must_still_expose_percent_tick_contract),
    ('Category 3 - Error/Negative Path', test_negative_ocr_pipeline_must_not_be_invoked),
